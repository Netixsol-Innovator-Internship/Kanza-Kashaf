...
const CartPopup = ({ onClose }) => {
  ...
  if (cartCount === 0) {
    return (
      <div className="fixed inset-0 z-50">
        ...
        {/* Popup */}
        <div ref={popupRef} className="absolute top-0 right-0 w-full max-w-sm sm:max-w-md lg:max-w-lg bg-white dark:bg-gray-800 shadow-lg border dark:border-gray-700 flex flex-col transition-colors">
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      {/* Popup */}
      <div ref={popupRef} className="absolute top-0 right-0 w-full max-w-sm sm:max-w-md lg:max-w-lg bg-white dark:bg-gray-800 shadow-lg border dark:border-gray-700 flex flex-col transition-colors h-full sm:h-auto sm:max-h-screen">
          ...
          {cartItems.map((item) => (
            <div key={item.product._id} className="flex items-start justify-between gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <button onClick={() => removeFromCart(item.product._id)} className="text-xs text-gray-800 dark:text-gray-300 mt-1 hover:text-red-600 dark:hover:text-red-400 transition-colors">
                  REMOVE
                </button>
              </div>
